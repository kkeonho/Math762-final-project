function [F_net] = deformed_1(p_ini,p,Np,Lx,Ly,Lb,dA,xi,K,num_fam,fam_ind,v,omega,Ib,lambda,mu,P,nnn,cstr)
...
%             FF = FF + v(j,fam_ind(j,k))*dA*omega(j,fam_ind(j,k))*eta(k,2*j-1:2*j)'*xi(k,2*j-1:2*j)*(K(2*j-1:2*j,:))^-1;
            FF = FF + v(j,fam_ind(j,k))*dA*omega(j,fam_ind(j,k))*eta'*xi(k,2*j-1:2*j)*(K(2*j-1:2*j,:))^-1;
...
%         F(2*j-1:2*j,:) = FF;
    ...
    F_net = net_force_NOSB(p_ini,Np,num_fam,fam_ind,omega,v,PK1,K,xi,Lb,dA,Lx,Ly,P,nnn);
